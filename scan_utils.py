...
from folder_utils import calculate_folder_sizes
...
def scan_directory(directory: str, ignore_list: List[str], scan_type: str, max_depth: int) -> Tuple[List[Tuple[str, int, str, int]], List[Tuple[str, int]]]:
    ...
    folder_sizes = calculate_folder_sizes(directory, max_depth)
    large_folders = [(folder, size) for folder, size in folder_sizes.items()]
